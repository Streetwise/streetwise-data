...
def register_entry(image_keys, coord_list, key_canton, i, ir, image_angles, sequence_info):
    ...
    """
    myfile = open("data/ch_data.csv", "a")
    index_image=0
    while index_image + 16 < len(image_keys):
        download_image_by_key(image_keys[index_image], ir, "images/img" + str(i) + ".jpg")
...
#Geohashes of the urban and rural areas used to crawl images. Key correspond ot the cantons, RU is for rural
gh_cantons={"ZH":{"area_gh":"u0q", "suffix":["jd2","jd1", "j6r","jd8","j9","nnx","nr3", "k8t"]},
            ...
            "SH": {"area_gh":"u0qp", "suffix":["k6", "ks", "kf","7b","ky"]},
...
for key_canton in gh_cantons.keys():
    ...
    while area_canton < len(gh_cantons[key_canton]["suffix"]):
        ...
        raw_json = map.search_sequences(bbox = gh_box_map, per_page=10,start_time= start_date)
